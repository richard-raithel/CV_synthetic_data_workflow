...
class ImageComposition:
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        for fg in foregrounds:
            ...
            fg_image = self._transform_foreground(fg, fg_path, src_image_size)
            ...
            try:
                ...
                padding = 40
                radius = (min(composite.size[0], composite.size[1]) - padding) / 2

                # Check if the fg_image is too big
                if max(fg_image.size[0], fg_image.size[1]) > 2 * radius:
                    print(
                        f'Foreground {fg_path} is too big ({fg_image.size[0]}x{fg_image.size[1]}) for the circular region with radius {radius}. Skipping this image.')
                    continue  # Skip to the next iteration of the loop

                # If the fg_image is not too big, continue with the placement
                radius -= max(fg_image.size[0], fg_image.size[1]) / 2

                # Determine a random angle and distance from the center
                angle = 2 * math.pi * random.random()  # Angle in radians
                distance = radius * math.sqrt(
                    random.random())  # Random distance from center, weighted towards the center

                # Convert polar coordinates to Cartesian for the offset from the center
                offset_x = distance * math.cos(angle)
                offset_y = distance * math.sin(angle)

                # Determine the center of the image
                center_x = composite.size[0] / 2
                center_y = composite.size[1] / 2

                # Determine the upper left corner of the smaller image
                min_x_position = int(center_x + offset_x - fg_image.size[0] / 2)
                min_y_position = int(center_y + offset_y - fg_image.size[1] / 2)
                ...
                while onboarder == True:
                    paste_position = (min_x_position, min_y_position)
        ...
        for cnt in range(len(contours)):
            if hierarchy[0, cnt, 3] != -1:  # if the contour has a parent
                color = (0, 0, 0)  # set color to black
            else:
                color = colors[cnt]
            cv2.drawContours(np_image, contours, cnt, color, thickness=cv2.FILLED)
    ...
    def _transform_foreground(self, fg, fg_path, background_size):
        ...
        scale = tuple(dimA/dimB for dimA, dimB in zip(fg_image.size, background_size))
        ...
        # brightness_factor = random.random() * .5 + 2  # Pick something between .7 and 1.1
...
if __name__ == '__main__':
    ...
    DATASET_SIZE_TRAIN = int(dataset_size * split_size)

    val_size = 1-split_size
    if val_size > 0:
        DATASET_SIZE_VAL = int(dataset_size * val_size)
    else:
    ...
    # train_output = Path(os.path.join(base_path, f"train{dataset_size}"))
